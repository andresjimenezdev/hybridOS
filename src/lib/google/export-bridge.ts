import { AI_DATA_HEADERS, AI_HEALTH_HEADERS, AI_WEEKLY_HEADERS } from "./contracts";
...
type StrengthSession = {
  id: string; name: string; started_at: string; planned_sessions: { session_key: string | null } | null;
  strength_exercise_logs: Array<{ feeling: number | null; has_pain: boolean; notes: string | null; exercises: { name: string; external_key: string | null } | null; strength_sets: Array<{ set_number: number; weight_kg: number | null; reps: number | null; rir: number | null; completed: boolean }> }>;
};
type Cardio = { id: string; kind: string; performed_at: string; duration_seconds: number; distance_km: number | null; average_pace_seconds: number | null; average_heart_rate: number | null; max_heart_rate: number | null; rpe: number | null; feeling: number | null; notes: string | null; planned_sessions: { session_key: string | null; title: string } | null };
type Health = { measured_on: string; weight_kg: number | null; body_fat_percent: number | null; resting_heart_rate: number | null; vo2_max: number | null; steps: number | null; active_calories: number | null; total_calories: number | null; sleep_minutes: number | null };
...
type Planned = { scheduled_date: string; kind: string; status: string };
...
export async function exportBridgeTables(supabase: Supabase, userId: string) {
  const [strengthResult, cardioResult, healthResult, measurementResult, plannedResult] = await Promise.all([
    supabase.from("strength_sessions").select("id,name,started_at,planned_sessions(session_key),strength_exercise_logs(feeling,has_pain,notes,exercises(name,external_key),strength_sets(set_number,weight_kg,reps,rir,completed))").eq("user_id", userId).eq("status", "completed").order("started_at"),
    supabase.from("cardio_sessions").select("id,kind,performed_at,duration_seconds,distance_km,average_pace_seconds,average_heart_rate,max_heart_rate,rpe,feeling,notes,planned_sessions(session_key,title)").eq("user_id", userId).order("performed_at"),
    supabase.from("health_metrics").select("measured_on,weight_kg,body_fat_percent,resting_heart_rate,vo2_max,steps,active_calories,total_calories,sleep_minutes").eq("user_id", userId).order("measured_on"),
    ...
    supabase.from("planned_sessions").select("scheduled_date,kind,status").eq("user_id", userId).order("scheduled_date"),
  ]);
  ...
  for (const session of strength) {
    for (const log of session.strength_exercise_logs) {
      for (const set of log.strength_sets.filter((item) => item.completed).sort((a, b) => a.set_number - b.set_number)) {
        dataRows.push([
          session.started_at.slice(0, 10), "strength", value(session.planned_sessions?.session_key), session.name,
          value(log.exercises?.name), value(log.exercises?.external_key), set.set_number, value(set.weight_kg),
          value(set.reps), value(set.rir), value(log.feeling), log.has_pain, value(log.notes), "", "", "", "", "", "",
        ]);
      }
    }
  }
  for (const session of cardio) {
    dataRows.push([
      session.performed_at.slice(0, 10), session.kind, value(session.planned_sessions?.session_key),
      session.planned_sessions?.title ?? session.kind, "", "", "", "", "", "", value(session.feeling), "",
      value(session.notes), value(session.distance_km), session.duration_seconds, value(session.average_pace_seconds),
      value(session.average_heart_rate), value(session.max_heart_rate), value(session.rpe),
    ]);
  }
  ...
  for (const date of healthDates) {
    ...
    healthRows.push([
      ...
      value(metrics?.vo2_max), value(metrics?.steps), value(metrics?.active_calories), value(metrics?.total_calories),
  ...
  const cardioKinds = new Set(["running", "cycling_outdoor", "stationary_bike", "treadmill", "stair_machine", "walking", "other"]);
  ...
  for (const [weekStart, week] of [...weeks.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    const validPlanned = week.planned.filter((entry) => entry.status !== "cancelled" && entry.status !== "skipped");
    ...
    weeklyRows.push([
      weekStart,
      validPlanned.filter((entry) => entry.kind === "strength").length,
      validPlanned.filter((entry) => entry.kind === "strength" && entry.status === "completed").length,
      validPlanned.filter((entry) => cardioKinds.has(entry.kind)).length,
      validPlanned.filter((entry) => cardioKinds.has(entry.kind) && entry.status === "completed").length,
      runs.length,
...
export async function runOutboundSync(supabase: Supabase, userId: string, entityType: string, entityId: string | null) {
  ...
  try {
    const result = await exportBridgeTables(supabase, userId);
    ...
    const message = error instanceof Error ? error.message.slice(0, 500) : "Unknown Google sync error";

...
export default async function StrengthProgressPage() {
  ...
  return (
    <main><SectionHeading eyebrow="Progreso" title="Fuerza" action={<Link className="text-sm" href="/progreso">Volver</Link>} />
      ...
      <section className="mt-9"><p className="eyebrow mb-4">Sesiones recientes</p>{sessions.length ? <div className="space-y-3">{sessions.map((session) => <article className="card flex items-center justify-between p-5" key={session.id}><div><p className="font-semibold">{session.name}</p><p className="mt-1 text-xs text-[var(--muted)]">{new Intl.DateTimeFormat("es-ES", { day: "numeric", month: "short", year: "numeric" }).format(new Date(session.started_at))}</p></div><span className="text-sm text-[var(--muted)]">{session.duration_minutes ?? "—"} min</span></article>)}</div> : <div className="card p-6 text-sm text-[var(--muted)]">Aún no hay sesiones terminadas.</div>}</section>
    </main>
  );
}

      ...
      <section className="card overflow-hidden">
        ...
        <div className="border-t border-[var(--line)] px-3 py-4 sm:p-6">
          ...
          {current.sets.map((set) => (
            <div className="grid grid-cols-[1.5rem_minmax(0,1.15fr)_minmax(0,0.9fr)_minmax(0,0.8fr)_2.75rem] items-center gap-1.5 py-1.5 sm:gap-2" key={set.id}>
              ...
              <button aria-label={`Completar serie ${set.set_number}`} className={`h-12 rounded-xl border text-lg ${set.completed ? "border-[var(--success)] bg-[var(--success)] text-white" : "border-[var(--line)]"}`} onClick={() => updateSet(set.id, { completed: !set.completed })} type="button">✓</button>
            </div>
          ))}
        </div>

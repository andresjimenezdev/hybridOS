  ...
  return (
    <main className="mx-auto max-w-2xl">
      ...
      <section className="card overflow-hidden">
        ...
        <div className="border-t border-[var(--line)] p-4 sm:p-6"><div className="mb-2 grid grid-cols-[2rem_1fr_1fr_1fr_2.5rem] gap-2 px-1 text-center text-[0.65rem] font-semibold uppercase tracking-wider text-[var(--muted)]"><span>#</span><span>kg</span><span>{current.target_duration_min_seconds !== null ? "seg" : "reps"}</span><span>RIR</span><span>✓</span></div>{current.sets.map((set) => <div className="grid grid-cols-[2rem_1fr_1fr_1fr_2.5rem] items-center gap-2 py-2" key={set.id}><span className="text-center text-sm text-[var(--muted)]">{set.set_number}</span><div className="flex items-center rounded-xl border border-[var(--line)]"><button aria-label="Restar 2,5 kg" className="min-h-12 min-w-9 text-lg" onClick={() => updateSet(set.id, { weight_kg: Math.max(0, (set.weight_kg ?? 0) - 2.5) })} type="button">−</button><input aria-label={`Peso serie ${set.set_number}`} className="min-w-0 flex-1 bg-transparent text-center text-lg font-semibold outline-none" inputMode="decimal" onChange={(event) => updateSet(set.id, { weight_kg: event.target.value === "" ? null : Number(event.target.value) })} type="number" value={set.weight_kg ?? ""} /><button aria-label="Sumar 2,5 kg" className="min-h-12 min-w-9 text-lg" onClick={() => updateSet(set.id, { weight_kg: (set.weight_kg ?? 0) + 2.5 })} type="button">+</button></div><input aria-label={`${current.target_duration_min_seconds !== null ? "Segundos" : "Repeticiones"} serie ${set.set_number}`} className="field min-w-0 px-1 text-center text-lg font-semibold" inputMode="numeric" min="0" onChange={(event) => current.target_duration_min_seconds !== null ? updateSet(set.id, { duration_seconds: event.target.value === "" ? null : Number(event.target.value) }) : updateSet(set.id, { reps: event.target.value === "" ? null : Number(event.target.value) })} type="number" value={current.target_duration_min_seconds !== null ? set.duration_seconds ?? "" : set.reps ?? ""} /><input aria-label={`RIR serie ${set.set_number}`} className="field min-w-0 px-1 text-center text-lg font-semibold" inputMode="decimal" max="10" min="0" step="0.5" onChange={(event) => updateSet(set.id, { rir: event.target.value === "" ? null : Number(event.target.value) })} type="number" value={set.rir ?? ""} /><button aria-label={`Completar serie ${set.set_number}`} className={`min-h-12 rounded-xl border text-lg ${set.completed ? "border-[var(--success)] bg-[var(--success)] text-white" : "border-[var(--line)]"}`} onClick={() => updateSet(set.id, { completed: !set.completed })} type="button">✓</button></div>)}</div>

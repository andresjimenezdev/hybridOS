...
type CurrentLog = { id: string; exercise_id: string; position: number; target_sets: number | null; target_reps_min: number | null; target_reps_max: number | null; target_duration_min_seconds: number | null; target_duration_max_seconds: number | null; target_rir: number | null; target_rir_min: number | null; target_rir_max: number | null; rest_seconds: number | null; feeling: number | null; has_pain: boolean; notes: string | null; completed: boolean; exercises: { name: string } | null; strength_sets: WorkoutSet[] };
...
export default async function StrengthSessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const { data: logsData } = await supabase.from("strength_exercise_logs").select("id,exercise_id,position,target_sets,target_reps_min,target_reps_max,target_duration_min_seconds,target_duration_max_seconds,target_rir,target_rir_min,target_rir_max,rest_seconds,feeling,has_pain,notes,completed,exercises(name),strength_sets(id,set_number,weight_kg,reps,duration_seconds,rir,completed)").eq("strength_session_id", sessionId).order("position");

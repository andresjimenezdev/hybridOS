...
type PlannedExercise = {
  id: string; position: number; target_sets: number; target_reps_min: number | null; target_reps_max: number | null;
  target_duration_min_seconds: number | null; target_duration_max_seconds: number | null; target_rir: number | null;
  target_rir_min: number | null; target_rir_max: number | null; rest_seconds: number | null; exercises: { name: string } | null;
};
type Planned = {
  id: string; title: string; kind: string; scheduled_date: string; status: string; target_duration_minutes: number | null; target_distance_km: number | null;
  planned_session_exercises: PlannedExercise[];
  workout_templates: { workout_template_exercises: PlannedExercise[] } | null;
};
...
function exerciseTarget(exercise: PlannedExercise) {
  if (exercise.target_duration_min_seconds !== null) {
    return `${exercise.target_duration_min_seconds}${exercise.target_duration_max_seconds !== exercise.target_duration_min_seconds ? `–${exercise.target_duration_max_seconds}` : ""} s`;
  }
  if (exercise.target_reps_min !== null) {
    return `${exercise.target_reps_min}${exercise.target_reps_max !== exercise.target_reps_min ? `–${exercise.target_reps_max}` : ""} reps`;
  }
  return "Objetivo libre";
}

function ExerciseDetails({ session }: { session: Planned }) {
  if (session.kind !== "strength") return null;
  const snapshot = session.planned_session_exercises;
  const exercises = (snapshot.length ? snapshot : session.workout_templates?.workout_template_exercises ?? []).toSorted((a, b) => a.position - b.position);
  if (!exercises.length) return <p className="mt-4 border-t border-[var(--line)] pt-4 text-xs text-[var(--muted)]">Esta sesión todavía no tiene ejercicios asociados.</p>;
  return <details className="mt-4 border-t border-[var(--line)] pt-3"><summary className="flex min-h-10 items-center justify-between text-sm font-medium"><span>Ver ejercicios</span><span className="text-xs text-[var(--muted)]">{exercises.length}</span></summary><ol className="space-y-2 pt-2">{exercises.map((exercise) => { const rirMin = exercise.target_rir_min ?? exercise.target_rir; const rirMax = exercise.target_rir_max ?? exercise.target_rir; return <li className="flex items-start justify-between gap-4 rounded-xl bg-black/[0.025] px-3 py-3 dark:bg-white/[0.04]" key={exercise.id}><div className="flex min-w-0 gap-3"><span className="text-xs tabular-nums text-[var(--muted)]">{exercise.position}</span><p className="truncate text-sm font-medium">{exercise.exercises?.name ?? "Ejercicio"}</p></div><div className="shrink-0 text-right"><p className="text-xs font-semibold">{exercise.target_sets} × {exerciseTarget(exercise)}</p>{rirMin !== null ? <p className="mt-1 text-[0.65rem] text-[var(--muted)]">RIR {rirMin}{rirMax !== rirMin ? `–${rirMax}` : ""}{exercise.rest_seconds ? ` · ${exercise.rest_seconds} s descanso` : ""}</p> : exercise.rest_seconds ? <p className="mt-1 text-[0.65rem] text-[var(--muted)]">{exercise.rest_seconds} s descanso</p> : null}</div></li>; })}</ol></details>;
}
...
export default async function PlanningPage({ searchParams }: { searchParams: Promise<{ week?: string }> }) {
  ...
  const [{ data: plannedData }, { data: templateData }] = await Promise.all([
    supabase.from("planned_sessions").select("id,title,kind,scheduled_date,status,target_duration_minutes,target_distance_km,planned_session_exercises(id,position,target_sets,target_reps_min,target_reps_max,target_duration_min_seconds,target_duration_max_seconds,target_rir_min,target_rir_max,rest_seconds,exercises(name)),workout_templates(workout_template_exercises(id,position,target_sets,target_reps_min,target_reps_max,target_duration_min_seconds,target_duration_max_seconds,target_rir,target_rir_min,target_rir_max,rest_seconds,exercises(name)))")
  ...
  const sessions = (plannedData ?? []) as unknown as Planned[];
